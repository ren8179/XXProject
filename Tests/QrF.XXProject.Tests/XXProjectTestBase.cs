...
namespace QrF.XXProject.Tests
{
    public abstract class XXProjectTestBase : AbpIntegratedTestBase<XXProjectTestModule>
    {
        ...
        protected void UsingDbContext(Action<XXProjectDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Action<XXProjectDbContext> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<XXProjectDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<XXProjectDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<XXProjectDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<XXProjectDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Action<XXProjectDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<XXProjectDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<XXProjectDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<XXProjectDbContext, Task<T>> func)
        {
            T result;
